...
| `task`    | subagent  | Read-only agent for searching context and finding implementation details. |
...
Every sub-agent additionally loses `agent`, `agentic_fetch` and the
interactive `question` tool at run time, whatever its configuration says.
...
- **subagent** — Only launched via the `agent` tool. Cannot dispatch further
  sub-agents (depth = 1), and does not hold the `agentic_fetch` tool either,
  since that tool can also start an agent.
...
- `subagent_type` selects which agent to use. Defaults to `task` if omitted.
- The tool description dynamically lists all available sub-agents.
- Sub-agents cannot dispatch further sub-agents or use `agentic_fetch`
...
| `model`         | string          | `large`      | Model type (`large` or `small`)                  |
