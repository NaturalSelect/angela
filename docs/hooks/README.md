...
- Angela ships with a builtin `angela-hook` skill write, edit, and configure
  hooks; just tell Angela how to configure Angela
- Angela currently supports just one hook, `PreToolUse`, with plans to support
...
our **project-level** `angela.json`. Relative paths like `./no-haskell.sh` work
here because the project root is your working directory. If you're configuring
a global hook (`~/.config/angela/angela.json`), use an absolute path instead.
...
# ($ANGELA_TOOL_INPUT_COMMAND) to grep and match on a regexp.
if echo "$ANGELA_TOOL_INPUT_COMMAND" | grep -qE '(^| )((ghc|cabal|stack)(\.exe)?)( |$)'; then
...
Hooks run through Angela's embedded POSIX shell (`mvdan.cc/sh`) — the same
...
- **Permissive shebang fallback**: if the absolute path in a shebang
  doesn't exist (e.g. `#!/bin/bash` on Windows), Angela falls back to a
  ...
  "no opinion".
- **Environment**: every hook sees `ANGELA=1`, `AGENT=angela`, and
  `AI_AGENT=angela` on top of the `ANGELA_*` hook-specific variables. These
  ...
  both contexts.
- **Timeout behavior**: when a hook exceeds its timeout, Angela cancels the
  context and waits a short grace period (~1s) for the interpreter to
  yield. If the hook still hasn't returned, Angela abandons it, logs a
...
Hooks can be added to your `angela.json` (or `.angela.json`) at both the global
...
> work fine in project-level `angela.json` because the project root is also
> your working directory. For **global** config (`~/.config/angela/`),
> however, you must use either an absolute path or an inline command:
>
> ```jsonc
> // Global ~/.config/angela/angela.json
> {
>   "hooks": {
>     "PreToolUse": [
>       {
>         "command": "/home/you/.config/angela/hooks/no-haskell.sh"
...
3. Runs all matching hooks **in parallel** through Angela's embedded POSIX
...
5. Applies the result **before** permission checks. If the aggregated decision
   ...
   for it. If it's `allow`, Angela treats that as affirmative pre-approval and
...
however you'll incur some additional overhead as Angela will still parse and
run each hook.
...
| ---------------------------- | ---------------------------------------------- |
| `ANGELA`                      | Always `1` when running under Angela.           |
| `AGENT`                      | Always `angela`.                                |
| `AI_AGENT`                   | Always `angela`.                                |
| `ANGELA_EVENT`                | The hook event name (e.g. `PreToolUse`).       |
| `ANGELA_TOOL_NAME`            | The tool being called (e.g. `bash`).           |
...
The `ANGELA`, `AGENT`, and `AI_AGENT` markers are also set by the `bash`
tool, so a script can detect "am I running under Angela?" the same way in
either context.
...
Hooks communicate back to Angela via **exit code** and `stdout`/`stderr`. The
...
deterministic based on the order hooks appear in `angela.json`.
...
If a hook exceeds its timeout, Angela cancels its context and treats the
...
returns `decision: "allow"`, which tells Angela to pre-approve the call:
...
case "$ANGELA_TOOL_INPUT_COMMAND" in
...
```jsonc
{ "command": "echo \"$(date -Iseconds) $ANGELA_TOOL_NAME\" >> ./tools.log" }
```
...
Angela hooks are broadly compatible with [Claude Code
...
hooks run under Angela unchanged. This document covers the Angela-specific API
only — anything not documented here isn't guaranteed to work.

One intentional divergence: Angela treats `updated_input` as a shallow-merge
...
4. `decision` precedence: `deny` > `allow` > `null`. First deny determines the
   ...
   is `allow`, Angela pre-approves the tool call and skips the permission
