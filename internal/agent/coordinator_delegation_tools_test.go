...
// half: a sub-agent that has spent the configured options.subagent_depth
// budget (2 by default) must never receive the agent tool, regardless of
// what its AllowedTools configuration says, because that would let the
// dispatch chain grow past the budget. The primary coder agent, which runs
// at depth 0, must still get it when its AllowedTools allows it.
func TestSubagentsHaveNoDelegationTools(t *testing.T) {
	...
	for _, id := range []string{config.AgentExplore, config.AgentGeneral} {
		t.Run(id, func(t *testing.T) {
			names := toolNames(t, id, 2)
			require.NotContains(t, names, toolnames.Agent, "an out-of-budget sub-agent %q must not hold the agent tool", id)
		})
	}
...
// sub-agent that has spent its delegation budget (depth 2, the default)
// gets no agent tool at all, whether or not options.subagent_branches is
// on — that option only lifts branchDispatchRefusal's depth check, not the
// delegation budget itself.
