...
// event into a closure nobody reads, and subscribers (`angela run`)
...
// publish a RunComplete for its RunID, hanging a `angela run` caller that
...
// than internal bookkeeping, which is the contract a `angela run` caller
...
// bookkeeping slice, not the published event a `angela run` caller awaits.
