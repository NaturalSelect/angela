...
- `old_string` must match the file exactly, including indentation, and be unique — the edit fails otherwise. Strip the line-number prefix (right-aligned line number then `|`, e.g. `    12|`) before matching.
...
- For whole-function/method/type replacements prefer `LSPReplaceSymbol` (no whitespace matching needed). For renames prefer `LSPRename` (semantic, cross-file). For large edits use Write.
