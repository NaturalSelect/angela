...
func TestWriteToolWritesEmptyNewFile(t *testing.T) {
	...
	tool := NewWriteTool(nil, newHistoryService(t, "", false), newFileTracker(t, time.Now()), workingDir)
...
func writePlan(t *testing.T, workingDir string, params WriteParams) Plan {
	...
	tool := NewWriteTool(nil, newHistoryService(t, "", false), newFileTracker(t, time.Now()), workingDir).(*writeTool)
