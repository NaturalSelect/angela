...
func findModels(providers map[string]config.ProviderConfig, largeModel, smallModel string) ([]modelMatch, []modelMatch, error) {
	...
	for _, pf := range []struct {
		...
		{largeProviderFilter, string(config.ModelMain)},
		{smallProviderFilter, string(config.ModelChore)},
	} {
		if pf.filter != "" {
			if _, ok := providers[pf.filter]; !ok {
				return nil, nil, fmt.Errorf("%s model: provider %q not found in configuration. Use 'angela models' to list available models", pf.label, pf.filter)
			}
		}
	}
