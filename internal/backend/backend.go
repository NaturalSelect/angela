...
// Overridable via ANGELA_SERVER_IDLE_TIMEOUT (seconds; 0 restores the
...
// via ANGELA_SERVER_DETACH_GRACE (seconds; 0 restores immediate teardown).
...
// Backend provides transport-agnostic business logic for the Angela
...
func New(ctx context.Context, cfg *config.ConfigStore, shutdownFn ShutdownFunc) *Backend {
	return &Backend{
		...
		detachGrace: durationFromEnv("ANGELA_SERVER_DETACH_GRACE", DefaultDetachGrace),
	}
}

// idleShutdownDelayFromEnv returns the idle-shutdown delay, honoring a
// ANGELA_SERVER_IDLE_TIMEOUT override (in seconds; 0 disables lingering).
func idleShutdownDelayFromEnv() time.Duration {
	return durationFromEnv("ANGELA_SERVER_IDLE_TIMEOUT", DefaultIdleShutdownDelay)
}
