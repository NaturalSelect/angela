...
// with reason=tool_use and used to terminate `angela run` early via
...
// subscribers (e.g. a TUI watching workspace events while `angela
...
// non-nil error from `angela run` so shells and CI catch it via
// exit status.
...
// run (e.g. Ctrl+C while `angela run` waits) exits cleanly rather
...
// queue scenario: `angela run --continue` attaches to a session
