...
// instructions.
//
// Reads go through os.Root so the entry cannot be swapped for a link
// between listing and reading. Links in the components of dir itself
// are followed, and that is deliberate: rejecting them would disable
// discovery for anyone whose home, config dir or worktree is a
// symlink, while buying almost nothing — someone who can write inside
// a scanned directory can simply commit a regular agent file instead.
func DiscoverAgentFiles(paths []string) map[string]Agent {
	...
	for _, dir := range paths {
		...
		if err != nil {
			slog.Warn("Failed to open agent directory", "dir", dir, "error", err)
			continue
		}
		...
		for _, entry := range entries {
			...
			agent, err := parseAgentFile(root, entry.Name())
			if err != nil {
...
// the body becomes the Prompt. The read is relative to root, so a name
// swapped for a link after listing cannot reach outside it.
func parseAgentFile(root *os.Root, name string) (Agent, error) {
