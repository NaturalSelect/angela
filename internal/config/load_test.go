...
func TestLookupConfigs_BoundedByProject(t *testing.T) {
	// Force GlobalConfig to point at a location we control so it can be
	// present in the result without polluting the developer's real
	// config.
	globalDir := t.TempDir()
	t.Setenv("ANGELA_GLOBAL_CONFIG", globalDir)

	t.Run("does not pick up angela.json above non-git project", func(t *testing.T) {
		parent := t.TempDir()

		// angela.json above the project must not be adopted.
		require.NoError(t, os.WriteFile(
			filepath.Join(parent, "angela.json"),
			[]byte(`{}`),
			0o644,
		))
		...
		for _, p := range got {
			require.NotEqual(t, filepath.Join(parent, "angela.json"), p)
		}
	})

	t.Run("does not climb out of git worktree to find angela.json", func(t *testing.T) {
		...
		strayEval, err := filepath.EvalSymlinks(filepath.Join(parent, "angela.json"))
		require.NoError(t, err)
		for _, p := range got {
			...
			require.NotEqual(t, strayEval, pEval, "must not adopt parent angela.json")
		}
	})

	t.Run("picks up angela.json inside the project", func(t *testing.T) {
		project := t.TempDir()
		local := filepath.Join(project, "angela.json")
		...
		require.True(t, foundLocal, "expected project angela.json to be in lookup result: %v", got)
	})
	...
	t.Run("global config is always included regardless of boundary", func(t *testing.T) {
		...
		got := lookupConfigs(project)
		// Global config is always prepended, even when no project file
		// exists.
...
func TestConfig_setDefaults(t *testing.T) {
	t.Run("sets default data directory", func(t *testing.T) {
		...
		require.NotNil(t, cfg.Slots)
		require.NotNil(t, cfg.LSP)
		require.NotNil(t, cfg.MCP)
		require.Equal(t, filepath.Join(workingDir, ".angela"), cfg.Options.DataDirectory)
	...
	t.Run("does not adopt .angela from a parent project", func(t *testing.T) {
		parent := t.TempDir()

		// .angela in the parent: it should not be reused by the child
	...
	t.Run("does not climb out of git worktree to find .angela", func(t *testing.T) {
		if _, err := exec.LookPath("git"); err != nil {
			t.Skip("git not available")
		}
		...
		// Stray .angela above the worktree root.
		require.NoError(t, os.Mkdir(filepath.Join(parent, defaultDataDirectory), 0o755))
		...
		require.NoError(t, err)
		require.NotEqual(t, strayEval, gotEval, "must not adopt parent .angela")
		...
		cfg := &Config{
			Providers: csync.NewMapFrom(map[string]ProviderConfig{
				"custom": {
					...
					Models: []ProviderModel{
						{Model: catwalk.Model{ID: "existing-model", Name: "My Custom Name", ContextWindow: 200000}},
					},
		...
		cfg := &Config{
			Providers: csync.NewMapFrom(map[string]ProviderConfig{
				"custom": {
					...
					BaseURL: "https://api.custom.com/v1",
					Models: []ProviderModel{
						{Model: catwalk.Model{ID: "my-model", Name: "My Model"}},
					},
				},
			}),
		}
...
func TestConfig_defaultModelSelection(t *testing.T) {
	t.Run("default behavior uses the default models for given provider", func(t *testing.T) {
		...
		require.Equal(t, int64(1000), cfg.GetModel(large.Provider, large.Model).DefaultMaxTokens)
		require.Equal(t, "small-model", small.Model)
		require.Equal(t, "openai", small.Provider)
		require.Equal(t, int64(500), cfg.GetModel(small.Provider, small.Model).DefaultMaxTokens)
	})
	...
	t.Run("should configure the default models with a custom provider", func(t *testing.T) {
		...
		require.Equal(t, int64(600), cfg.GetModel(large.Provider, large.Model).DefaultMaxTokens)
		require.Equal(t, "model", small.Model)
		require.Equal(t, "custom", small.Provider)
		require.Equal(t, int64(600), cfg.GetModel(small.Provider, small.Model).DefaultMaxTokens)
	})
	...
	t.Run("when enabled, provider missing base_url is rejected", func(t *testing.T) {
		cfg := &Config{
			...
			Providers: csync.NewMapFrom(map[string]ProviderConfig{
				"my-llm": {
					APIKey: "test-key",
					Models: []ProviderModel{{Model: catwalk.Model{ID: "model"}}},
					// No BaseURL.
				},
			}),
		}
...
func TestConfig_setDefaultsDisableDefaultProvidersEnvVar(t *testing.T) {
	t.Run("sets option from environment variable", func(t *testing.T) {
		t.Setenv("ANGELA_DISABLE_DEFAULT_PROVIDERS", "true")
...
func TestConfig_configureSelectedModels(t *testing.T) {
	t.Run("reload mode should not persist fallback defaults", func(t *testing.T) {
		...
		require.NoError(t, os.WriteFile(globalPath, []byte(`{"slots":{"main":{"provider":"ghost","model":"missing"}}}`), 0o600))
		...
		cfg := &Config{
			Slots: map[SlotName]SelectedModel{
				SlotMain: {Provider: "ghost", Model: "missing"},
			},
		}
		cfg.setDefaults(dir, "")
		...
		require.Equal(t, "openai", cfg.Slots[SlotMain].Provider)
		require.Equal(t, "large-model", cfg.Slots[SlotMain].Model)
	...
	t.Run("should override defaults", func(t *testing.T) {
		...
		cfg.Slots[SlotMain] = resolved.Main
		cfg.Slots[SlotChore] = resolved.Chore
		large := cfg.Slots[SlotMain]
		small := cfg.Slots[SlotChore]
		require.Equal(t, "larger-model", large.Model)
		require.Equal(t, "openai", large.Provider)
		require.Equal(t, int64(2000), cfg.GetModel(large.Provider, large.Model).DefaultMaxTokens)
		require.Equal(t, "small-model", small.Model)
		require.Equal(t, "openai", small.Provider)
		require.Equal(t, int64(500), cfg.GetModel(small.Provider, small.Model).DefaultMaxTokens)
	})
	t.Run("should be possible to use multiple providers", func(t *testing.T) {
		...
		require.Equal(t, int64(200), cfg.GetModel(small.Provider, small.Model).DefaultMaxTokens)
	})
	t.Run("resolve and persist fallback under writeMu does not deadlock", func(t *testing.T) {
		...
		cfg := &Config{
			Slots: map[SlotName]SelectedModel{
				SlotMain:  {Provider: "openai", Model: "this-model-does-not-exist"},
				SlotChore: {Provider: "openai", Model: "also-does-not-exist"},
			},
		}
		cfg.setDefaults(dir, "")
		store := &ConfigStore{config: cfg, globalDataPath: globalPath}
		...
		case err := <-done:
			...
			require.Equal(t, "large-model", cfg.Slots[SlotMain].Model)
			require.Equal(t, "small-model", cfg.Slots[SlotChore].Model)
...
// TestSelectedModelVariantsSurviveResolution pins the whole point of
// declaring a variant in config: model resolution rewrites the entry it
// resolves, so anything it forgets to carry over is gone before the
// selector ever sees it.
func TestSelectedModelVariantsSurviveResolution(t *testing.T) {
	dir := t.TempDir()
	knownProviders := []catwalk.Provider{
		{
			ID:                  "openai",
	...
	cfg := &Config{
		...
		Slots: map[SlotName]SelectedModel{
			SlotMain: {Provider: "openai", Model: "large-model"},
		},
	}
	cfg.setDefaults(dir, "")
	store := &ConfigStore{config: cfg, globalDataPath: filepath.Join(dir, "angela.json")}
	e := env.NewFromMap(map[string]string{})
	require.NoError(t, cfg.configureProviders(
		context.Background(), store, e, NewShellVariableResolver(e), knownProviders))

	resolved, err := resolveSelectedModels(cfg, knownProviders)
	require.NoError(t, err)

	// Variants now live on the provider's catalog entry, keyed by model
	// ID, rather than on the resolved SelectedModel itself — so
	// surviving resolution means surviving the provider-configuration
	// merge that runs ahead of it.
	main := cfg.GetModel(resolved.Main.Provider, resolved.Main.Model)
	require.NotNil(t, main)
	require.Contains(t, main.Variants, "deep",
		"a variant declared on the main model must survive provider configuration")
	require.Equal(t, int64(32000), *main.Variants["deep"].MaxTokens)
}
	...
	cfg := &Config{
		Providers: csync.NewMapFrom(map[string]ProviderConfig{
			"my-llm": {
				...
				BaseURL: "https://my-llm.example.com/v1",
				Type:    catwalk.TypeOpenAI,
				Models:  []ProviderModel{{Model: catwalk.Model{ID: "m"}}},
				ExtraHeaders: map[string]string{
					"X-Custom": "",
					"X-Kept":   "present",
				},
