...
func TestLookupConfigs_BoundedByProject(t *testing.T) {
	...
	t.Setenv("ANGELA_GLOBAL_DATA", dataDir)
...
func TestConfig_setupAgentsWithEveryReadOnlyToolDisabled(t *testing.T) {
	...
	coderAgent, ok := cfg.Agents[AgentCoder]
	require.True(t, ok)
	assert.Equal(t, []string{"agent", "bash", "angela_info", "angela_logs", "job_output", "job_kill", "download", "edit", "multiedit", "lsp_diagnostics", "lsp_references", "lsp_restart", "lsp_rename", "lsp_replace_symbol", "fetch", "question", "todos", "write", "list_mcp_resources", "read_mcp_resource"}, coderAgent.AllowedTools.Tools)
...
func TestSelectedModelVariantsSurviveResolution(t *testing.T) {
	...
	require.Contains(t, resolved.Main.Variants, "deep",
		"a variant declared on the main model must survive resolution")
	require.Equal(t, int64(32000), *resolved.Main.Variants["deep"].MaxTokens)

	// The copy has to be independent: the resolved entry is published to
	// every reader while the source config stays writable.
	resolved.Main.Variants["extra"] = SelectedModelOverride{}
	require.NotContains(t, cfg.Models[ModelMain].Variants, "extra")
}
