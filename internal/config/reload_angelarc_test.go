...
// TestReloadFromDisk_PicksUpEditedAngelarc verifies that a angelarc edit on
// disk is reflected in memory after a reload, matching how JSON config
// already behaves.
func TestReloadFromDisk_PicksUpEditedAngelarc(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	rcPath := filepath.Join(workDir, "angelarc")
	...
	require.Equal(t, "osc", store.Config().Options.Notifications,
		"reload must re-execute the angelarc and pick up the new value")
}

// TestReloadFromDisk_FailingAngelarcKeepsOldConfig verifies that a angelarc
...
func TestReloadFromDisk_FailingAngelarcKeepsOldConfig(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	rcPath := filepath.Join(workDir, "angelarc")
	require.NoError(t, os.WriteFile(rcPath, []byte("option notifications bell\n"), 0o644))
	...
	// Overwrite with a angelarc that errors at load time (unknown option key).
	require.NoError(t, os.WriteFile(rcPath, []byte("option totally-bogus-key value\n"), 0o644))

	err = store.ReloadFromDisk(context.Background())
	require.Error(t, err, "a failing angelarc must surface a reload error")
	require.Equal(t, "bell", store.Config().Options.Notifications,
		"in-memory config must be preserved when a reload fails")
}

// TestReloadFromDisk_HangingAngelarcIsInterruptible is the core regression test
// for the reload-path blocker: config reloads run while holding the store's
// write lock, so a angelarc that blocks (busy loop, hung command substitution)
...
func TestReloadFromDisk_HangingAngelarcIsInterruptible(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	rcPath := filepath.Join(workDir, "angelarc")
	require.NoError(t, os.WriteFile(rcPath, []byte("option notifications bell\n"), 0o644))
	...
	// Overwrite with a angelarc that never terminates.
...
// TestLoad_TracksNotYetCreatedGlobalAngelarc verifies that config.Load tracks
// the global angelarc path even when the file does not exist yet, so a angelarc
// created after startup is detected as a staleness change. Previously only
// successfully-loaded paths were tracked, so a mid-session global angelarc went
// unnoticed until something else triggered a reload.
//
// Project-level angelarc files are discovered by walking the tree for existing
// files, so a not-yet-created project angelarc is still only picked up on the
// next reload; the global path is the common case this covers.
func TestLoad_TracksNotYetCreatedGlobalAngelarc(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	globalRC := filepath.Join(t.TempDir(), "angelarc")
	t.Setenv("ANGELA_GLOBAL_CONFIG", filepath.Dir(globalRC))
