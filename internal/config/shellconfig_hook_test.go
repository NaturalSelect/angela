...
func TestShellConfigHookAdd(t *testing.T) {
	store := loadAngelaSh(t, `hook add PreToolUse --matcher "^bash$" --command "echo hi" --name greet --timeout 10`)
...
func TestShellConfigHookRemoveByName(t *testing.T) {
	store := loadAngelaSh(t, `hook add PreToolUse --command a --name keep
...
func TestShellConfigHookClearEvent(t *testing.T) {
	store := loadAngelaSh(t, `hook add PreToolUse --command a --name a
