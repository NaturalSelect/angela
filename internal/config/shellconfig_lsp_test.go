...
func TestShellConfigLSPAdd(t *testing.T) {
	store := loadAngelaSh(t, `lsp add gopls --command gopls --filetypes go --filetypes mod --root-markers go.mod --timeout 60`)
...
func TestShellConfigLSPRemove(t *testing.T) {
	store := loadAngelaSh(t, `lsp add keepls --command keep-server
