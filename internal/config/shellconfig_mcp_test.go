...
func TestShellConfigMCPAdd(t *testing.T) {
	store := loadAngelaSh(t, `mcp add github --type http --url "https://api.githubcopilot.com/mcp/" --header Authorization "Bearer xyz"
...
func TestShellConfigMCPRemove(t *testing.T) {
	store := loadAngelaSh(t, `mcp add keep --command x
