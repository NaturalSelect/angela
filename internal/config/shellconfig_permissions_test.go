...
// loadAngelaSh writes an angela.sh into an isolated project and loads it through
...
func loadAngelaSh(t *testing.T, script string) *config.ConfigStore {
	t.Helper()
	store, err := loadAngelaShErr(t, script)
	require.NoError(t, err)
	return store
}

// loadAngelaShErr is loadAngelaSh without asserting success, for cases that are
// expected to fail at load time.
func loadAngelaShErr(t *testing.T, script string) (*config.ConfigStore, error) {
	t.Helper()
	...
	workDir := t.TempDir()
	dataDir := t.TempDir()
	require.NoError(t, os.WriteFile(filepath.Join(workDir, "angelarc"), []byte(script), 0o644))

	return config.Load(workDir, dataDir, false)
}

func TestShellConfigPermissionsAllow(t *testing.T) {
	store := loadAngelaSh(t, `permissions allow bash view`)
...
func TestShellConfigPermissionsAccumulateAndDedup(t *testing.T) {
	store := loadAngelaSh(t, `permissions allow bash
...
func TestShellConfigPermissionsDeny(t *testing.T) {
	store := loadAngelaSh(t, `permissions deny bash sourcegraph
