...
// already in use by another angela process.
var ErrDataDirLocked = errors.New("data directory already in use by another angela process")

// dataDirLockFile is the name of the lock file inside the data
// directory. It lives next to angela.db so users can `ls` and find it.
const dataDirLockFile = "angela.lock"
...
// {dataDir}/angela.lock. If the lock is already held by another
...
// ANGELA_SKIP_DATADIR_LOCK is set to a truthy value. This is intended
...
func skipDataDirLock() bool {
	v, _ := strconv.ParseBool(os.Getenv("ANGELA_SKIP_DATADIR_LOCK"))
	return v
}
