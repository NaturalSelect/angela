...
INSERT INTO sessions (
    ...
    strftime('%s', 'now'),
    strftime('%s', 'now')
) RETURNING id, parent_session_id, title, message_count, prompt_tokens, completion_tokens, cost, updated_at, created_at, summary_message_id, todos, agent, model
`
...
SELECT id, parent_session_id, title, message_count, prompt_tokens, completion_tokens, cost, updated_at, created_at, summary_message_id, todos, agent, model
FROM sessions
...
const updateSessionAgentAndModel = `-- name: UpdateSessionAgentAndModel :exec
UPDATE sessions
SET
    agent = ?,
    model = ?,
...
type UpdateSessionAgentAndModelParams struct {
	Agent sql.NullString `json:"agent"`
	Model sql.NullString `json:"model"`
	ID    string         `json:"id"`
}

func (q *Queries) UpdateSessionAgentAndModel(ctx context.Context, arg UpdateSessionAgentAndModelParams) error {
	_, err := q.exec(ctx, q.updateSessionAgentAndModelStmt, updateSessionAgentAndModel, arg.Agent, arg.Model, arg.ID)
	return err
}
