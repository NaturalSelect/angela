...
func TestAngelaIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .angelaignore file that ignores .log files
	require.NoError(t, os.WriteFile(".angelaignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .angelaignore file
	angelaignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".angelaignore"), []byte(angelaignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .angelaignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .angelaignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .angelaignore in subdir that ignores normal_nested
	subAngelaignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".angelaignore"), []byte(subAngelaignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .angelaignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .angelaignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .angelaignore")
