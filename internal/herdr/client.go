...
// multiplexer. When Angela runs inside a herdr-managed pane it reports
...
// Client reports Angela agent state to a running herdr instance.
type Client struct {
	socketPath string
	paneID     string
...
// call from environment variables. Returns nil when Angela is not
...
// is not strictly greater. Because angela seeds seq from the wall
// clock at startup (see newFromEnv), a restarted angela in the same
...
func (c *Client) newRequestLocked(method, idPrefix, state string) reportRequest {
	...
	return reportRequest{
		ID:     fmt.Sprintf("angela:%s:%d", idPrefix, time.Now().UnixNano()),
		Method: method,
		Params: reportParams{
			PaneID:         c.paneID,
			Source:         "angela",
