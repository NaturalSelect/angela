...
func TestBuildEnv(t *testing.T) {
	...
	require.Equal(t, EventPreToolUse, envMap["ANGELA_EVENT"])
	require.Equal(t, "bash", envMap["ANGELA_TOOL_NAME"])
	require.Equal(t, "sess-1", envMap["ANGELA_SESSION_ID"])
	require.Equal(t, "/work", envMap["ANGELA_CWD"])
	...
	// scripts can.
	require.Equal(t, "1", envMap["ANGELA"])
	require.Equal(t, "angela", envMap["AGENT"])
	require.Equal(t, "angela", envMap["AI_AGENT"])
}
...
func TestParseStdoutClaudeCodeFormat(t *testing.T) {
	...
	t.Run("native format still works", func(t *testing.T) {
		t.Parallel()
