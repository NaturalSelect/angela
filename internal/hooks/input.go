...
func BuildEnv(eventName, toolName, sessionID, cwd, projectDir, toolInputJSON string, agent AgentIdentity) []string {
	...
	env = append(
		env,
		fmt.Sprintf("ANGELA_EVENT=%s", eventName),
		fmt.Sprintf("ANGELA_TOOL_NAME=%s", toolName),
	...
	if toolInputJSON != "" {
		if cmd := gjson.Get(toolInputJSON, "command"); cmd.Exists() {
			env = append(env, fmt.Sprintf("ANGELA_TOOL_INPUT_COMMAND=%s", cmd.String()))
		}
		if fp := gjson.Get(toolInputJSON, "file_path"); fp.Exists() {
			env = append(env, fmt.Sprintf("ANGELA_TOOL_INPUT_FILE_PATH=%s", fp.String()))
		}
	}
...
// Supports both Angela format and Claude Code format (hookSpecificOutput).
