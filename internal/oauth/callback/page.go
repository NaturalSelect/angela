...
func Write(w io.Writer, r Result) error {
	...
	if r.Failed() {
		data.Title = "Authorization failed — Angela"
		data.Kind = "failed"
		data.Heading = "Authorization failed"
		data.Detail = "Angela was not granted access to"
		if r.Subject == "" {
			data.Detail = "Angela was not granted access."
		}
		// A failed page keeps itself open: the reader needs the reason,
		// and closing the tab out from under them would take it away.
		data.Status = "Close this tab and try again from Angela."
	} else {
		data.Title = "Authorized — Angela"
		data.Kind = "ok"
		data.Heading = "You’re all set"
		data.Detail = "Angela is now connected to"
		if r.Subject == "" {
			data.Detail = "Angela is now connected."
		}
