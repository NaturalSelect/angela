...
// gitUnsafeOptions make git run an external program or point it at a
// different repository, which voids the read-only classification.
var gitUnsafeOptions = []string{
	"--config-env", "--exec-path", "--ext-diff", "--git-dir",
	"--open-files-in-pager", "--output", "--pager", "--receive-pack",
	"--textconv", "--upload-pack", "--work-tree", "-C", "-O", "-c",
}
