...
func TestKnownBypassesAreNotSafe(t *testing.T) {
	...
	cases := []struct {
		...
		{"git pager option glued to its value runs a program", "git grep -Otouch-evil TODO"},
		{"git -c overrides configuration", "git -c core.pager=evil status"},
		{"git -c glued to its value overrides configuration", "git -ccore.pager=evil status"},
		{"git -C glued to its value points at another repo", "git -C/etc status"},
