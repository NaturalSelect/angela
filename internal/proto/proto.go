...
type Workspace struct {
	...
	YoloMerge bool `json:"yolo_merge,omitempty"`
...
// UpdateAvailable is published when a newer Angela release is detected
...
// completion contract (notably `angela run` in client/server mode)
...
// `angela run`, which may attach to a busy session whose currently
