...
// TestE2EAttachingClearsTheUnattendedMark drives the handover the
// headless mark would otherwise poison. `angela run --continue` marks
// its session so prompts are refused rather than parked; against a
// persistent server that mark outlives the run. When a TUI later opens
// the same session it reports itself as viewing it, and from that
// moment there is someone who can answer — so the refusal must stop.
//
// Without this the user is told "nothing is attached to this session"
// while looking straight at it.
func TestE2EAttachingClearsTheUnattendedMark(t *testing.T) {
	h := newE2EHarness(t)
	ctx, cancel := context.WithTimeout(t.Context(), 30*time.Second)
	defer cancel()

	const sessionID = "s-handover"

	// The headless run marks the session on its way past.
	h.setSessionUnattended(t, ctx, h.workspace.ID, sessionID, true)
	require.True(t, h.app.Permissions.SessionUnattended(sessionID))

	// An interactive client opens a real stream and reports the session
	// it is looking at, exactly as the TUI does on loadSession.
	clientID := uuid.NewString()
	_, stop := h.subscribeSSE(t, ctx, h.workspace.ID, clientID)
	defer stop()
	h.waitForAttached(t, 1)

	require.NoError(t,
		h.backend.SetCurrentSession(h.workspace.ID, clientID, sessionID))

	require.False(t, h.app.Permissions.SessionUnattended(sessionID),
		"a client watching the session can answer for it")

	// And the gate now reaches a prompt rather than refusing outright.
	// Denying it proves a decision was asked for, not short-circuited.
	events := h.app.Permissions.Subscribe(ctx)
	done := make(chan permission.Decision, 1)
	go func() {
		done <- h.app.Permissions.Gate(ctx, permission.GateRequest{
			SessionID:  sessionID,
			ToolCallID: "tc-handover",
			Access: permission.Access{
				Tool:    "bash",
				Action:  permission.ActionExecute,
				Command: "rm -rf build",
	...
	select {
	case ev := <-events:
		h.app.Permissions.Deny(ev.Payload)
	case <-time.After(5 * time.Second):
		t.Fatal("the gate never asked; the stale mark still short-circuits it")
	}

	select {
	case decision := <-done:
		require.False(t, decision.Allowed())
		require.NotContains(t, decision.Reason, "attached",
			"this was a real denial, not a refusal for want of an audience")
	case <-time.After(5 * time.Second):
		t.Fatal("the gate never settled after the denial")
	}
}
