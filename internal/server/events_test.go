...
// highlighted code from read, diffs from edit, images, etc.) and
...
// are what non-interactive clients (e.g. `angela run`) rely on to
...
// through the SSE envelope. Without these correlators, `angela run`
...
// "agent failed" (returns non-zero from `angela run`) from "agent
