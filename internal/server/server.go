...
import (
	...
	_ "github.com/NaturalSelect/angela/internal/swagger"
...
// socketDir returns the directory used for the Angela Unix socket.
...
// returned by [socketDir] and is named angela-<uid>.sock, falling
// back to angela.sock when the current uid cannot be determined. If
// the composed path would exceed [maxUnixSocketPathLen] bytes (the
// macOS sun_path limit), we fall back to /tmp/angela-<uid>.sock so
// the socket remains bindable.
func DefaultHost() string {
	sock := "angela.sock"
	usr, err := user.Current()
	if err == nil && usr.Uid != "" {
		sock = fmt.Sprintf("angela-%s.sock", usr.Uid)
	}
...
// Server represents a Angela server bound to a specific address.
type Server struct {
...
func (s *Server) installHandler() {
	...
	mux.HandleFunc("POST /v1/workspaces/{id}/client-image-support", c.handlePostWorkspaceClientImageSupport)
	...
	mux.HandleFunc("GET /v1/workspaces/{id}/sessions/{sid}/undo", c.handleGetWorkspaceSessionUndo)
	mux.HandleFunc("POST /v1/workspaces/{id}/sessions/{sid}/undo", c.handlePostWorkspaceSessionUndo)
	...
	mux.HandleFunc("POST /v1/workspaces/{id}/agent/sessions/{sid}/abandon-branch", c.handlePostWorkspaceAgentSessionAbandonBranch)
	...
	mux.HandleFunc("POST /v1/workspaces/{id}/agent/sessions/{sid}/prompts/take", c.handlePostWorkspaceAgentSessionPromptTake)
	mux.HandleFunc("POST /v1/workspaces/{id}/agent/sessions/{sid}/summarize", c.handlePostWorkspaceAgentSessionSummarize)
	mux.HandleFunc("GET /v1/workspaces/{id}/agent/active-agent", c.handleGetWorkspaceAgentDefaultActiveAgent)
	mux.HandleFunc("POST /v1/workspaces/{id}/agent/active-agent", c.handlePostWorkspaceAgentDefaultActiveAgent)
	mux.HandleFunc("GET /v1/workspaces/{id}/agent/sessions/{sid}/active-agent", c.handleGetWorkspaceAgentSessionActiveAgent)
	mux.HandleFunc("POST /v1/workspaces/{id}/agent/sessions/{sid}/active-agent", c.handlePostWorkspaceAgentSessionActiveAgent)
	mux.HandleFunc("POST /v1/workspaces/{id}/agent/sessions/{sid}/shell", c.handlePostWorkspaceAgentSessionShell)
	mux.HandleFunc("POST /v1/workspaces/{id}/agent/sessions/{sid}/side-question", c.handlePostWorkspaceAgentSessionSideQuestion)
	mux.HandleFunc("POST /v1/workspaces/{id}/agent/sessions/{sid}/commit-message", c.handlePostWorkspaceAgentSessionCommitMessage)
	...
	mux.HandleFunc("POST /v1/workspaces/{id}/config/provider-model", c.handlePostWorkspaceConfigProviderModel)
	...
	mux.HandleFunc("POST /v1/workspaces/{id}/config/agent-model", c.handlePostWorkspaceConfigAgentModel)
	mux.HandleFunc("DELETE /v1/workspaces/{id}/config/agent-model", c.handleDeleteWorkspaceConfigAgentModel)
