...
func TestRegisterBuiltin_DispatchedThroughRun(t *testing.T) {
	RegisterBuiltin("angelatest-echo", func(ctx context.Context, args []string, stdin io.Reader, stdout, stderr io.Writer) error {
		io.WriteString(stdout, "builtin:"+args[1])
		return nil
	})
	t.Cleanup(func() { delete(builtins, "angelatest-echo") })

	var stdout bytes.Buffer
	err := Run(t.Context(), RunOptions{
		Command: "angelatest-echo hello",
		Cwd:     t.TempDir(),
		Stdout:  &stdout,
	})
	if err != nil {
...
func TestRegisterBuiltin_OverridesPATHBinary(t *testing.T) {
	dir := t.TempDir()
	// Put a fake "angelatest-shadowed" binary on PATH that would print
	// "from-path" if it ran.
	binDir := filepath.Join(dir, "bin")
	if err := os.MkdirAll(binDir, 0o755); err != nil {
		t.Fatal(err)
	}
	script := filepath.Join(binDir, "angelatest-shadowed")
	if err := os.WriteFile(script, []byte("#!/bin/sh\necho from-path\n"), 0o755); err != nil {
		t.Fatal(err)
	}
	t.Setenv("PATH", binDir+string(os.PathListSeparator)+os.Getenv("PATH"))

	RegisterBuiltin("angelatest-shadowed", func(ctx context.Context, args []string, stdin io.Reader, stdout, stderr io.Writer) error {
		io.WriteString(stdout, "from-builtin")
		return nil
	})
	t.Cleanup(func() { delete(builtins, "angelatest-shadowed") })

	var stdout bytes.Buffer
	err := Run(t.Context(), RunOptions{
		Command: "angelatest-shadowed",
		Cwd:     dir,
		Env:     os.Environ(),
		Stdout:  &stdout,
	})
