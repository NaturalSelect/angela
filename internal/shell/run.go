...
type RunOptions struct {
	...
	// the Angela process cwd — hooks and the bash tool have different
...
// Angela handler stack. Shared by the stateless [Run] entrypoint and the
...
// standard Angela middleware chain (builtins, script dispatch, block list)
...
// when sourcing framework files can send SIGINT/SIGTERM to Angela's process
...
// regardless of the caller's environment because Angela shells are never
...
//  1. builtins first (so Angela's in-process jq wins over any PATH binary);
