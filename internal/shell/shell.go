...
func NewShell(opts *Options) *Shell {
	...
	// binaries and nested angela instances) can't attach to or release
	// the parent pane's agent authority.
	env = withoutHerdrEnv(env)

	// Allow tools to detect execution by Angela.
	env = append(env, AngelaEnvMarkers()...)
