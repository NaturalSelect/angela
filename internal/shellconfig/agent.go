...
func agentAdd(b *ConfigBuilder, args []string, stderr io.Writer) error {
	...
	name := args[2]
	if err := checkAgentName("agent add", name, stderr); err != nil {
		return err
	}
...
// agentIDPattern mirrors the rule config.ValidateAgentID enforces on
// markdown-defined agents. It is duplicated rather than imported
// because config loads this package, so the dependency cannot run the
// other way.
var agentIDPattern = regexp.MustCompile(`^[a-z0-9]+(?:-[a-z0-9]+)*$`)

// checkAgentName rejects a name the rest of the system would refuse,
// at the line that wrote it. Accepting it here buries the mistake in a
// config key that silently never resolves to an agent.
func checkAgentName(cmd, name string, stderr io.Writer) error {
	if !agentIDPattern.MatchString(name) {
		return usage(stderr, fmt.Sprintf("%s: invalid agent name %q: must be lowercase alphanumeric segments separated by single hyphens", cmd, name))
	}
	return nil
}
...
func agentRemove(b *ConfigBuilder, args []string, stderr io.Writer) error {
	...
	if err := checkAgentName("agent remove", name, stderr); err != nil {
		return err
	}
	m := childMap(b.section("agents"), name)
	clear(m)
