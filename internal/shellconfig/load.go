...
// loadTimeout bounds a single angelarc execution. Config loading runs on the
...
// LoadShellConfig executes an angelarc script and returns its config as a
...
func LoadShellConfig(ctx context.Context, path string, src []byte) ([]byte, error) {
	...
	// Expose the running Angela version so scripts can feature-detect, e.g.
	// [[ "$ANGELA_VERSION" == "devel" ]] or branch on the release.
	env := append(os.Environ(), "ANGELA_VERSION="+version.Version)
