...
// TestLoadShellConfig_AngelaVersionEnv verifies that ANGELA_VERSION is exposed
// to the script so it can feature-detect the running Angela version.
func TestLoadShellConfig_AngelaVersionEnv(t *testing.T) {
	dir := t.TempDir()
	script := `provider add openai --api-key "$ANGELA_VERSION"`
	path := filepath.Join(dir, "angelarc")
...
// angelarc cannot block config loading indefinitely. Config loads run on the
...
func TestLoadShellConfig_RespectsContextCancellation(t *testing.T) {
	...
	select {
	case err := <-done:
		require.Error(t, err, "a cancelled angelarc must fail, not succeed")
