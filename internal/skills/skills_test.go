...
func TestToPromptXMLBuiltinType(t *testing.T) {
	...
	skills := []*Skill{
		{Name: "builtin-skill", Description: "A builtin.", SkillFilePath: "angela://skills/builtin-skill/SKILL.md", Builtin: true},
		{Name: "user-skill", Description: "A user skill.", SkillFilePath: "/home/user/.config/angela/skills/user-skill/SKILL.md"},
	}
	xml := ToPromptXML(skills)
...
func TestDiscoverBuiltin(t *testing.T) {
	...
	for _, s := range discovered {
		if s.Name == "angela-config" {
			...
			require.Equal(t, "angela://skills/angela-config/SKILL.md", s.SkillFilePath)
			require.Equal(t, "angela://skills/angela-config", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	}
	require.True(t, found, "angela-config builtin skill not found")

	var foundJQ bool
	for _, s := range discovered {
		if s.Name == "jq" {
			foundJQ = true
			require.Equal(t, "angela://skills/jq/SKILL.md", s.SkillFilePath)
			require.Equal(t, "angela://skills/jq", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	...
	var foundHooks bool
	for _, s := range discovered {
		if s.Name == "angela-hooks" {
			foundHooks = true
			require.Equal(t, "angela://skills/angela-hooks/SKILL.md", s.SkillFilePath)
			require.Equal(t, "angela://skills/angela-hooks", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
...
func TestDeduplicate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			input:    []*Skill{{Name: "angela-config", Path: "angela://skills/angela-config"}, {Name: "angela-config", Path: "/user/angela-config"}},
			wantLen:  1,
			wantName: "angela-config",
			wantPath: "/user/angela-config",
		},
		{
			name:    "empty",
			input:   nil,
