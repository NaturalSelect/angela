...
func TestTracker_BuiltinSkillTracking(t *testing.T) {
	...
	// Simulate active skills including a builtin skill (angela-config).
	activeSkills := []*Skill{
		{Name: "angela-config", Description: "Angela config", Builtin: true},
		{Name: "go-doc", Description: "Go docs", Builtin: false},
	}
	tracker := NewTracker(activeSkills)

	// Initially not loaded.
	require.False(t, tracker.IsLoaded("angela-config"))
	require.False(t, tracker.IsLoaded("go-doc"))

	// Mark builtin skill as loaded (simulating read via angela://...).
	tracker.MarkLoaded("angela-config")
	require.True(t, tracker.IsLoaded("angela-config"))
