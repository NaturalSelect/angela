...
// (creations), as a percentage rounded to the nearest point. Plain,
// never-cached input tokens are excluded from both sides of the ratio:
...
func CacheHitRate(readTokens, creationTokens int64) (pct int64, ok bool) {
	total := readTokens + creationTokens
	if total <= 0 {
		return 0, false
	}
	return int64(math.Round(float64(readTokens) / float64(total) * 100)), true
}
