...
// TestMCPServers_CursorAccountsForWideRunes verifies the filter
// input's screen cursor lands past double-width runes (CJK text) by
// their real display width, not by rune count.
func TestMCPServers_CursorAccountsForWideRunes(t *testing.T) {
	t.Parallel()

	dASCII := newTestMCPServers(t, nil)
	for _, r := range "abc" {
